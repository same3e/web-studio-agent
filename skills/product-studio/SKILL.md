...
8a. For implementation, review, testing, or approved refactoring, resolve the active stack, applicable code-quality modules, existing repository conventions, material approved deviations, and required validation commands. Security, correctness, data integrity, factual constraints, and approval constraints remain mandatory.
8b. Determine lean, standard, or thorough workflow mode; initialize or maintain the compact project snapshot without overwriting user edits; refresh the deterministic repository map and resolved rules when invalidated; compile role context before specialist work and route the next role from its compact delta. Context packages select pointers and summaries, never replace canonical records or runtime evidence.
9. Read machine-readable project state, scope, surfaces, and approval records; initialize only applicable records, populate them, run machine-readable preflight that blocks implementation on missing or inconsistent required evidence. Preserve legacy records without inventing facts. Implement only approved active scope.
