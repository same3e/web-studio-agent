...
- **Report:** every finding has `Severity`, `Evidence`, `Affected file`, `Classification` (approved project requirement / global plugin default / recommendation / accepted risk / not applicable), and `Recommended remediation`; inspect token usage, duplicated arbitrary values, component structure, unauthorized dependencies, invented content, scope expansion, and declared states. It must not claim a layout looks good without browser evidence.
