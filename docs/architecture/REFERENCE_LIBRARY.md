...
The same home root may contain `taste/PROFILE.md`, initialized only when absent by `scripts/Initialize-TasteProfile.ps1`. It is a user-controlled cross-project preference record, separate from the reference library and never a source of project facts.
